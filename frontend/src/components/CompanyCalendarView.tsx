...
export default function CompanyCalendarView({ token }: { token: string }) {
  ...
  const loadDays = async (keepDate?: string) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error((data as any).message || 'Nie udało się pobrać kalendarza');
      setDays(data);
      const targetDate = keepDate || selectedDate;
      const matchingDay = data.find((item) => item.date === targetDate);
      if (matchingDay) {
        setIsWorkingDay(matchingDay.isWorkingDay);
        setReason(matchingDay.reason || '');
      }
  ...
  return <div style={{ height: '100%', overflow: 'auto' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: 'minmax(280px, 1fr) minmax(280px, 1fr)', gap: '1rem' }}>
      <div className="card">
        <h3 style={{ marginBottom: '1rem' }}>Dni w zakresie</h3>
        {loading ? (
          <p>Ładowanie...</p>
        ) : (
          days.map((day) => (
            <button
              ...
              style={{
                ...
                textAlign: 'left',
                border: day.date === selectedDate ? '1px solid var(--primary-color)' : '1px solid var(--border-color)',
            ...
            >
              ...
              <span
                style={{
                  color: day.isWorkingDay ? 'var(--success-color)' : 'var(--danger-color)',
                  fontWeight: 600,
                  flexShrink: 0,
                  marginLeft: '0.5rem',
                }}
              >
                {day.isWorkingDay ? 'roboczy' : 'wolny'}
              </span>
            </button>
          ))
        )}
