...
- Diagnostyka podaje faktyczną przyczynę wykluczenia wpisu z rozliczenia: dla typów z `requiresOrder=true` bez zlecenia wskazuje `Brak zlecenia`, a dla typów z `requiresOrder=false` i `isAbsence=false` wskazuje `Typ nie jest nieobecnością i nie wymaga zlecenia`.
