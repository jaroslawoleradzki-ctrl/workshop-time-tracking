...
Backend używa Vitest i Supertest. `backend/tests/version.test.ts` sprawdza `/api/version`; `integration.test.ts` sprawdza health check (200 lub 503), odrzucenie błędnego logowania oraz ochronę tras bez tokenu. `copy-last-day.test.ts` sprawdza kontrakt i walidację kopiowania, wybór pracownika i źródła, soft delete, limit, role, rollback audytu oraz serializację 2 i 20 równoległych żądań na deterministycznym modelu transakcji. `duplicate-report-classifier.test.ts` sprawdza konserwatywną klasyfikację duplikatów, pełny snapshot rekordu dla v2, prawidłowe kopiowanie, równoległe partie, lawinowe powielanie, regresję powtórzonych sesji importu z odstępami kilku minut i zachowanie oryginalnej sesji bez wcześniejszego dnia źródłowego, typy bez zlecenia i rozróżnianie godzin. `repair-manifest-builder.test.ts` sprawdza manifest v2, fingerprint i normalizację, dokładnego poprzednika KEEP, rekonstrukcję poprzedników dla powtórzonych sesji importu, odfiltrowanie kandydatów REVIEW/DELETE przed oceną liczby, dwóch kandydatów KEEP, brak kwalifikowanego poprzednika, batch mieszany, zgodność `reportIds` oraz deterministyczność. `duplicate-repair-executor.test.ts` sprawdza summary/approve v1 i v2, zachowanie preconditions, atomowość zapisu, execute stub oraz walidację fingerprintów, poprzedników, konfliktów i approval.
