...
- Dowód `REPEATED_IMPORT_SESSION` z detektora v2 jest traktowany jako historia kopiowania/importu na równi z innymi silnymi dowodami. Builder rekonstruuje relację `repeatedImportSessionOf`: pierwszą stabilną, audytowaną sesję może oznaczyć jako `KEEP`, a późniejsze sesje mogą wskazać jej rekordy jako poprzedników. Builder nadal wymaga pełnych preconditions i dokładnie jednego wcześniejszego poprzednika z batcha `KEEP`; brak takiego poprzednika pozostawia batch w `REVIEW`.
