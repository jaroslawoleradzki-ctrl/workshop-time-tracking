...
- **Orders**: Zlecenia produkcyjne z planem godzinowym (`plannedHours`), ilością (`quantity`), jednostką (`quantityUnit`), opcjonalnym kodem produktu (`productCode`), nazwą produktu, opcjonalnym kontem księgowym, statusami (`OPEN`, `SUSPENDED`, `CLOSED`) oraz flagą aktywności `isActive`.
